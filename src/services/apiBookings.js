...
export async function getStaysTodayActivity() {
  const { data, error } = await supabase
    ...
    .or(`and(status.eq.unconfirmed,startDate.gte.${getToday()},startDate.lte.${getToday({ end: true })}),and(status.eq.checked-in,endDate.gte.${getToday()},endDate.lte.${getToday({ end: true })})`)
    .order('created_at')
